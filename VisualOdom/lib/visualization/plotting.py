...
def visualize_paths(gt_path, pred_path, html_tile="", title="VO exercises", file_out="plot.html"):
    ...
    # FIX: Agregar radius a los círculos
    circle_radius = max(np.ptp(gt_path[:, 0]), np.ptp(gt_path[:, 1])) * 0.01  # 1% del rango de datos
    
    fig1.circle("gtx", "gty", source=source, color="blue", hover_fill_color="firebrick", 
               legend_label="GT", radius=circle_radius)
    fig1.line("gtx", "gty", source=source, color="blue", legend_label="GT")

    fig1.circle("px", "py", source=source, color="green", hover_fill_color="firebrick", 
               legend_label="Pred", radius=circle_radius)
    ...
    # FIX: Agregar radius al círculo de error también
    error_radius = len(diff) * 0.01  # 1% del número de frames
    fig2.circle("diffx", "diffy", source=source, hover_fill_color="firebrick", 
               legend_label="Error", radius=error_radius)
